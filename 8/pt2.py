...
def find_antinodes(map_dict, node_chars):
    ...
    for node_char_type in node_chars:
        ...
        for duo in node_duos:
            ...
            i = 0
            while True:
                antinode = [duo[0][0] - (i*rows_diff), duo[0][1] - (i*cols_diff)]
                if antinode[0] < 0 or antinode[0] >= max_rows or antinode[1] < 0 or antinode[1] >= max_cols:
                    break
                if check_antinode(antinode, map_dict, max_rows, max_cols):
                    num_antinodes += 1
                    map_dict[antinode[0]][antinode[1]] = antinode_char
                i += 1
